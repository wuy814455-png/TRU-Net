...
class Res34_Unet_NoTerrain(nn.Module):
    def __init__(self, inputchannel, outputchannel, BN_enable=True, resnet_pretrain=True, output_size=(161, 161)):
        super().__init__()
        self.BN_enable = BN_enable
        self.resnet = resnet34(pretrained=resnet_pretrain)
        filters = [64, 64, 128, 256, 512]  # ResNet34 的通道数
    ...
    def forward(self, x):
        # 气象特征编码
        x = self.firstconv(x)  # [batch_size, inputchannel, 33, 33] -> [batch_size, 64, 33, 33]
        x = self.firstbn(x)
        x = self.firstrelu(x)
        x_ = self.firstmaxpool(x)  # [batch_size, 64, 33, 33]
        e1 = self.encoder1(x_)  # [batch_size, 64, 33, 33]
        e2 = self.encoder2(e1)  # [batch_size, 128, 17, 17]
        e3 = self.encoder3(e2)  # [batch_size, 256, 9, 9]
        e4 = self.encoder4(e3)  # [batch_size, 512, 5, 5]
        center = self.center(e4)  # [batch_size, 512, 5, 5]
        center = tnf.interpolate(center, size=(e3.size(2), e3.size(3)), mode='bilinear', align_corners=False)  # [batch_size, 512, 9, 9]
        d1 = self.decoder0(torch.cat([center, e3], dim=1))  # [batch_size, 512+256, 9, 9] -> [batch_size, 256, 17, 17]
        d1 = tnf.interpolate(d1, size=(e2.size(2), e2.size(3)), mode='bilinear', align_corners=False)  # [batch_size, 256, 17, 17]
        d2 = self.decoder1(torch.cat([d1, e2], dim=1))  # [batch_size, 256+128, 17, 17] -> [batch_size, 128, 33, 33]
        d2 = tnf.interpolate(d2, size=(e1.size(2), e1.size(3)), mode='bilinear', align_corners=False)  # [batch_size, 128, 33, 33]
        d3 = self.decoder2(torch.cat([d2, e1], dim=1))  # [batch_size, 128+64, 33, 33] -> [batch_size, 64, 66, 66]
        d3 = self.dropout(d3)
        output = self.final_upsample(d3)  # [batch_size, 64, 66, 66] -> [batch_size, 1, 161, 161]
        return output

class ModifiedRes34_Unet_NoTerrain(Res34_Unet_NoTerrain):
    def __init__(self, inputchannel, outputchannel, BN_enable=True, resnet_pretrain=True, output_size=(161, 161)):
        super().__init__(inputchannel, outputchannel, BN_enable=BN_enable, resnet_pretrain=resnet_pretrain, output_size=output_size)

    c = Model(a)
    print(f"Output shape: {c.shape}")  # 应输出 torch.Size([5, 1, 161, 161])
